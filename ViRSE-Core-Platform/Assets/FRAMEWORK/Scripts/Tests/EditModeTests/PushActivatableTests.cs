...
namespace ViRSE.Tests
{
    public class PushActivatableTests
    {
        ...
        public void PushActivatable_WhenClicked_EmitsToPlugin()
        {
            ...
            ViRSENetworkSerializable state = new SingleInteractorActivatableState();
            WorldStateSyncer syncer = Substitute.For<WorldStateSyncer>();

            PushActivatable pushActivatable =
                PushActivatableFactory.Create(config, state, syncer, ServerType.Offline, "debug");
