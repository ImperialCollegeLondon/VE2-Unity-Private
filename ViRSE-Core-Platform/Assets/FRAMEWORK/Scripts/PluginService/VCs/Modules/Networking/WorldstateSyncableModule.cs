...
public class WorldstateSyncableModule : MonoBehaviour, IWorldStateSyncableModule
{
    ...
    float IWorldStateSyncableModule.SyncFrequency { get => _config.SyncFrequency; set => _config.SyncFrequency = value; }
    ...
    protected BaseSyncableState _state;
    ...
    public UnityEvent<BaseSyncableState> OnStateReceive { get; private set; } = new();

    public void Initialize(WorldStateSyncableConfig config, BaseSyncableState state)
    {
        _config = config;
        _state = state;
    ...
    private IEnumerator TriggerRegistrationAtEndOfFrame()
    {
        ...
        SyncableStateReceiveEvent stateReceivedFromSyncer = WorldStateSyncService.RegisterForSyncDataReceivedEvents(ID);
        stateReceivedFromSyncer.AddListener(OnReceiveStateFromSyncer);
    }

    protected virtual void OnReceiveStateFromSyncer(BaseSyncableState receivedState)
    {
        OnStateReceive?.Invoke(receivedState);
    }

    protected virtual void FixedUpdate() 
    {
        bool onBroadcastFrame = InstanceSyncService.IsHost && (StaticData.fixedUpdateFrame + _hostSyncOffset) % _hostSyncInterval == 0;
        ...
        if (ID != null && (onBroadcastFrame || _forceTransmit))
        {
            ...
            if (_state != null) //PluginSyncables that haven't yet received state will be null
                WorldStateSyncService.AddStateToOutgoingBuffer(_state, _config.protocolConfig.TransmissionType);
        }
